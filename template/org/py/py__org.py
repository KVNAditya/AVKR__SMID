...
from template.org.py.models import ClientRegister_Model,Social_Media,detection_ratio,detection_accuracy
...
def Find_Social_Media_News_Type_Ratio(request):
    ...
    return render(request, 'Find_Social_Media_News_Type_Ratio.html', {'objs': obj})

def View_Remote_Users(request):
    obj=ClientRegister_Model.objects.all()
    return render(request,'View_Remote_Users.html',{'objects':obj})

def ViewTrendings(request):
    topic = Social_Media.objects.values('topics').annotate(dcount=Count('topics')).order_by('-dcount')
    return  render(request,'ViewTrendings.html',{'objects':topic})

def charts(request,chart_type):
    chart1 = detection_ratio.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"charts.html", {'form':chart1, 'chart_type':chart_type})

def charts1(request,chart_type):
    chart1 = detection_accuracy.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"charts1.html", {'form':chart1, 'chart_type':chart_type})

def Predict_Social_Media_News_Type(request):
    obj =Social_Media.objects.all()
    return render(request, 'Predict_Social_Media_News_Type.html', {'list_objects': obj})

def likeschart(request,like_chart):
    charts =detection_accuracy.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"likeschart.html", {'form':charts, 'like_chart':like_chart})
...
def Download_Trained_DataSets(request):
    ...
    # decide file name
    response['Content-Disposition'] = 'attachment; filename="trained_model.xls"'
    # creating workbook
...
def Train_Test_DataSets(request):
    ...
    return render(request,'Train_Test_DataSets.html', {'objs': obj})